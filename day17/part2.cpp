...
void solve(vector<vector<int>>& grid, int si, int sj) {
    ...
    while (!pq.empty()) {
        ...
        for (auto d : dirMap[node.dir]) {
            ...
            int newCnt = node.dir == dirMap2[d] ? node.cnt + 1 : 1;
            // cout << newCnt << endl;
            if (valid(ni, nj, grid) && newCnt <= 10 && (node.dir == dirMap2[d] || node.cnt >= 4 || node.cnt == 0)) {
                Node temp = Node(ni, nj, newCnt, node.cost + grid[ni][nj], dirMap2[d]);
                pq.push(temp);
            }
        }
    }
